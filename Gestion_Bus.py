...
def arret_de_nom(lieu):
    '''Fonction qui renvoie, s'il existe, l'arrêt avec le nom donné
    en argument, sinon un message d'erreur'''
...
def trajet_le_plus_rapide(l_itin_possibles):
    ...
    duree_tot = 0 
    for elt in l_itin_possibles[0]:
        duree_tot += duree(elt[1],elt[2])
    temps_min=duree_tot
    for itin in l_itin_possibles:
        duree_tot = 0 
        for elt in itin :
            duree_tot += duree(elt[1],elt[2])
        if duree_tot < temps_min:
            temps_min = duree_tot
            traj_plus_rapide = itin
